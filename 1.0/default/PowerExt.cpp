...
#define LOG_TAG "vendor.sprd.hardware.power@1.0-impl"
...
PowerExt::PowerExt(power_module_t *module) : mModule(module) {
    if (mModule)
        mModule->init(mModule);
}

PowerExt::~PowerExt() {
    ALOGD("%s: mModule %p", __func__, mModule);
    //delete(mModule);
}

// Methods from ::android::hardware::power::V1_0::IPower follow.
Return<void> PowerExt::setInteractive(bool interactive) {
    if (mModule->setInteractive)
        mModule->setInteractive(mModule, interactive ? 1 : 0);
    return Void();
}

Return<void> PowerExt::powerHint(PowerHint hint, int32_t data) {
    int32_t param = data;
...
Return<void> PowerExt::setFeature(Feature feature, bool activate) {
...
Return<void> PowerExt::getPlatformLowPowerStats(getPlatformLowPowerStats_cb _hidl_cb) {
...
Return<void> PowerExt::powerHintExt(PowerHintExt hint, int32_t data) {
    int32_t param = data;
    if (mModule->powerHint) {
        if (data)
            mModule->powerHint(mModule, static_cast<power_hint_t>(hint), &param);
...
IPowerExt* HIDL_FETCH_IPowerExt(const char* /* name */) {
    ...
    return new PowerExt(power_module);
}
